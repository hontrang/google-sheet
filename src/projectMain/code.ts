...
function getDataHose(): void {
  ...
  new SheetHelper().xoaDuLieuTrongCot(SheetHelper.SheetName.SHEET_THAM_CHIEU, 'A', 1, 4);

  for (const element of DANH_SACH_MA) {
    const price = priceMap[element] || 0;
    new SheetHelper().ghiDuLieuVaoDayTheoVung([[element]], SheetHelper.SheetName.SHEET_THAM_CHIEU, `A${indexSheetThamChieu}`);
    new SheetHelper().ghiDuLieuVaoDayTheoVung([[price]], SheetHelper.SheetName.SHEET_DU_LIEU, `B${indexSheetDuLieu}`);
    indexSheetDuLieu++;
    indexSheetThamChieu++;
  }
}
...
function layTinTucSheetBangThongTin(): void {
  ...
  const danhSachMa: string[] = new SheetHelper().layDuLieuTrongCot(SheetHelper.SheetName.SHEET_CAU_HINH, 'E');
  ...
  new SheetHelper().ghiDuLieuVaoDayTheoTen(mangDuLieuChinh, SheetHelper.SheetName.SHEET_BANG_THONG_TIN, 35, 'A');
}

function layThongTinChiTietMa(): void {
  new SheetHelper().ghiDuLieuVaoO('...', SheetHelper.SheetName.SHEET_CHI_TIET_MA, 'J2');
  new SheetHelper().xoaDuLieuTrongCot(SheetHelper.SheetName.SHEET_DU_LIEU, 'P', 3, 2);
  const tenMa: string = new SheetHelper().layDuLieuTrongO(SheetHelper.SheetName.SHEET_CHI_TIET_MA, 'F1');
...
function layGiaVaKhoiLuongTheoMaChungKhoan(tenMa = 'FRT'): void {
  const fromDate: string = new SheetHelper().layDuLieuTrongO(SheetHelper.SheetName.SHEET_CHI_TIET_MA, 'F2');
  const toDate: string = new SheetHelper().layDuLieuTrongO(SheetHelper.SheetName.SHEET_CHI_TIET_MA, 'H2');
  let index = 2;
  new SheetHelper().ghiDuLieuVaoDayTheoVung([['chi tiết mã', '', '']], SheetHelper.SheetName.SHEET_DU_LIEU, 'P1:R1');
  ...
  for (const element of datas) {
    new SheetHelper().ghiDuLieuVaoDayTheoVung([[element.date, element.close * 1000, element.nmVolume]], SheetHelper.SheetName.SHEET_DU_LIEU, `P${index}:R${index}`);
    index++;
  }
}
...
function layTinTucSheetChiTietMa(tenMa = 'FRT'): void {
  ...
  $('a').each(function (this: any) {
    ...
    new SheetHelper().ghiDuLieuVaoDayTheoVung([[tenMa.toUpperCase(), title, link, date]], SheetHelper.SheetName.SHEET_DU_LIEU, `AH${index}:AK${index}`);
    index++;
  });
}
...
function layBaoCaoTaiChinh(tenMa = 'FRT'): void {
  ...
  $('#divDocument>div>table>tbody>tr').each(function (this: any) {
    ...
    if (title !== 'Loại báo cáo' && index < 28) {
      new SheetHelper().ghiDuLieuVaoDayTheoVung([[tenMa.toUpperCase(), title, date, link]], SheetHelper.SheetName.SHEET_DU_LIEU, `AH${index}:AK${index}`);
      index++;
    }
  });
}
...
function layThongTinCoDong(tenMa = 'FRT'): void {
  ...
  new SheetHelper().ghiDuLieuVaoDayTheoTen(mangDuLieuChinh, SheetHelper.SheetName.SHEET_DU_LIEU, 2, 'AD');
}
...
function layThongTongSoLuongCoPhieuDangNiemYet(tenMa = 'FRT'): void {
  ...
  new SheetHelper().ghiDuLieuVaoDayTheoTen([[[object.data[0].RepeatedInfo[0].ListedShare]]], SheetHelper.SheetName.SHEET_CHI_TIET_MA, 18, 'H');
}
...
function layHeSoBetaVaFreeFloat(tenMa = 'FRT') {
  ...
  for (const element of datas) {
    if (element.ratioCode === 'BETA') {
      ...
      new SheetHelper().ghiDuLieuVaoO(value, SheetHelper.SheetName.SHEET_CHI_TIET_MA, 'E18');
    }
    if (element.ratioCode === 'FREEFLOAT') {
      const value = element.value;
      new SheetHelper().ghiDuLieuVaoO(value, SheetHelper.SheetName.SHEET_CHI_TIET_MA, 'J16');
    }
  }
}

...
/******/ (() => {
        ...
        formatDateTimeFromString(dt, fmt) {
          const knownEnglish = this.loc.listingMode() === 'en',
            ...
            weekday = (length, standalone) =>
              knownEnglish ? weekdayForDateTime(dt, length) : string(standalone ? { weekday: length } : { weekday: length, month: 'long', day: 'numeric' }, 'weekday'),
            maybeMacro = (token) => {
  ...
  let luxon = __webpack_require__(81);
  /**
   * Return write arguments.
   */
  __webpack_require__.g.luxon = luxon;
  /******/
})();
